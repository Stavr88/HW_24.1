...
class CourseViewSet(ModelViewSet):
    ...
    def course_update(self, serializer):
...
class SubscriptionCreateApiView(ListAPIView):
    queryset = Subscription.objects.all()
    serializer_class = SubscriptionSerializer
    permission_classes = [IsAuthenticated]

    def post(self, request, *args, **kwargs):
        """Метод отдает ответ в зависимости от действия."""
        ...
        if subs_item.exists():
            ...
            Subscription.objects.create(user=user, course=course_item, status_of_subscription=True)  # Создаем подписку
